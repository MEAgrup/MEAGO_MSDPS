...
| **MCN MEA — Foundation & Core** | `mcn_creators`, `app_config`, `platform_alerts`, ingest pipeline, `live_schedule_slots`, `brand_deals`, deals routing, `acquisitions`, `referrals`, `special_projects` | ✅ **STEP 1–3 COMPLETE**. (0300) Enum extensions (CreatorManagement, Acquisition divisi); (0301) app_config KV store + platform_alerts; (0302) mcn_creators master (sep dari KOL M9); (0303) ingest batches + 3 agregates (drop-raw); (0304) live_schedule (multi-slot, status done=locked, verify gate); (0305) brand_deals (shop_id unique nullable, merchant M4 optional) + cooperating_shops sync + FK; (0306) creator_requests (sample/ads/hsl+approval gate) + campaign_requests (CM→brand→final state machine); (0307) acquisitions (binding, commission snapshot, GMV windows) + referrals (antar/platform guard); (0308) special_projects (SPJ code, assignment split cm/acq, v_project_summary view). DB: 9 migr (0300–0308) ✅. Lib: 7 pure modules (parsers, weeks, file-read, ingest, routing, indicators, copy-week) ✅. Actions: 7 server action files (mcn-creators, deals, mcn-schedule, mcn-ingest, projects, acquisition, bizdev) ✅. **⏳ PENDING**: UI pages (10+), typecheck, smoke-test SQL, apply to live. |
