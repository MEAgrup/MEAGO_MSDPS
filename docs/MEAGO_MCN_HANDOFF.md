...
9. ~~**Advisor keamanan (opsional, pra-produksi)**~~ — ✅ **SELESAI ditinjau satu-per-satu** (2026-07-18, sesi orchestrator; saat ditinjau flag ERROR sudah 7 view karena +`v_portal_merchants`/`v_portal_deals` dari F.1/F.2). Hasil: **6 definer DISENGAJA, dibiarkan** — `v_merchant_board`/`v_speed_score`/`v_okr_attainment`/`v_management_dashboard` (role-gate eksplisit di WHERE, komentar migrasi 0206–0210) + `v_portal_merchants`/`v_portal_deals` (sengaja lewati RLS karyawan utk sesi kreator, kolom aman + gate `auth_creator_id()`); **1 diperbaiki** — migrasi **0313** flip `v_team_portal_tasks` → `security_invoker` (terkonfirmasi tidak sengaja; 2 saudaranya invoker). 0313 **applied ke live** via MCP `apply_migration` 2026-07-18 setelah smoke test transaksi-rollback PASS (hasil view identik definer vs invoker utk akun is_od & akun PIC 3 baris; rollback terverifikasi), QC pasca-apply PASS (PIC tetap 3 baris; advisor ERROR 7→6). WARN fungsi definer & INFO deny-all = by design (tetap).
