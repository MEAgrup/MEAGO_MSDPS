...
7. **Sisa platform** (di luar PR #5): ~~Merchant Portal Fase 2~~ — ❌ **TIDAK AKAN DIBUAT** (keputusan Yohan 2026-07-18, final — jangan diangkat lagi); rotasi password akun dummy (ditunda ke pra-produksi, keputusan 2026-07-16); ~~aktifkan *leaked password protection* di Supabase Auth~~ — ✅ **SELESAI** (diaktifkan di dashboard, konfirmasi Nerissa 2026-07-18).
...
9. ~~**Advisor keamanan (opsional, pra-produksi)**~~ — ✅ **TINJAUAN SELESAI** (2026-07-18, sesi orchestrator; migrasi **`0313_security_definer_view_review.sql`**). Hasil per view:
   - `v_merchant_board` — **KEEP definer (disengaja)**: role-gate di WHERE (`is_od/is_director/Account/divisi`), memang harus lintas-RLS tabel briefs/services/merchants/unit divisi.
   - `v_management_dashboard` — **KEEP definer (disengaja)**: gate `where is_od() or is_director()`.
   - `v_okr_attainment` — **KEEP definer (disengaja)**: gate WHERE cermin policy `okr_select` (komentar 0209 sudah eksplisit).
   - `v_speed_score` — **KEEP definer (wajib)**: proyeksi ber-gate dari `v_speed_score_internal` yang di-revoke dari authenticated (0208); bila invoker, user biasa tak bisa membaca sama sekali.
   - `v_team_portal_tasks` — **FLIP ke `security_invoker=true`**: satu-satunya yang tidak disengaja (saudaranya `_performance`/`_blocks` invoker sejak 0210). Flip tidak mengubah perilaku: basisnya `v_merchant_board` tetap definer ber-gate + granted ke authenticated; WHERE (auth.uid()/auth_division()) identik di kedua mode; kreator portal tetap 0 baris.
   Komentar 4 view yang dipertahankan diberi penanda **"SECURITY DEFINER DISENGAJA — jangan flip"** agar audit berikutnya tidak mengangkat ulang. WARN fungsi definer & INFO deny-all = by design. ⏳ **apply `0313` ke live via SQL Editor** (pola 0309/0312); sesudahnya advisor tetap menampilkan 4 ERROR definer lama + `v_portal_deals` — semuanya by design, abaikan.
