...
**Update 2026-07-18 — F.2 LIVE + QA PASS**: migrasi 0312 di-apply Yohan via SQL Editor (verifikasi 16 objek OK; catatan: TIDAK tercatat di riwayat `list_migrations`, preseden 0309), PR #9 merged ke production, QA e2e Yohan **berhasil** (portal + sisi karyawan). Advisor: satu-satunya temuan baru `v_portal_deals` definer view = by design. Keputusan tambahan: **Merchant Portal TIDAK akan dibuat** (final). Sisa housekeeping: hapus akun QA `kreator.qa@meago.dev` + data `qa-dummy` + report/komplain uji, rotasi akun dummy karyawan, ~~leaked password protection~~ ✅ **selesai** (diaktifkan di dashboard, konfirmasi Nerissa 2026-07-18), ~~tinjau 5 view definer lama~~ ✅ **selesai** (migrasi 0313 2026-07-18: 4 view dipertahankan definer-disengaja + komentar penanda, `v_team_portal_tasks` di-flip `security_invoker=true` — detail di `docs/MEAGO_MCN_HANDOFF.md` §8; ⏳ apply 0313 ke live via SQL Editor).
